...
**R4 に `address-family ipv4 vrf fabd2` が無いのは正常**（設定漏れではない）。
問題文「IaaSサイトは、このタスクの対象に含まれない」が理由で、R4 の CE 向け Gi1.100 / Gi1.414
（100.4.30.x）は IaaS 拠点行き。CML ラボでも R4 の物理リンクは `R4-Gi8<->R2-Gi0/1` の 1 本のみ。

### R1/R2 を neighbor に入れない理由（BGP-free core）

問題文 2 章「**R1およびR2では、BGPを動作させてはならない**」が直接の根拠。
`100.255.254.x` は全機の Lo0（末尾=機器番号）で、`.1`/`.2` だけ飛ぶのは **PE か P かで切れている**から。

- **P は外側ラベル 1 枚しか見ない**（内側の VPN ラベルも IP ヘッダも見ない）ので VRF も BGP も不要。
  転送: R3 が VPN ラベル + トランスポートラベルを 2 枚 push → R1 swap → R2 pop(PHP) → R6 が VPN ラベル pop。
- **「経路上にいる」≠「ピアである」**: R3↔R6 の iBGP は物理的に R1/R2 を通過するが、
  R1/R2 は TCP を中継するだけ。iBGP は `update-source Loopback0` のマルチホップなので間に何台あってもよい。
- ただし **R1/R2 が OSPF で Lo0 を配らないと iBGP は張れない**（= Task 1.7 が前提である理由）。
  P は BGP に「参加しない」が「不可欠」。
- **3 台なのは RR 禁止（フルメッシュ）だから**。PE 4 台 → 各 PE は自分以外の 3 台と張る（4C2 = 6 セッション）。
- 検算: 3(iBGP) + 2(eBGP) = **5** = 問題文「R3 にはアクティブな BGP ピアが 5 つ」と一致。
  R1/R2 を入れたら 7 本になり要件違反。

### 前提: Task 1.7 を先に完了させること（最重要）

R3 と R5 の **Lo0 は初期コンフィグで OSPF 未参加**（R5 は OSPF プロセス自体が無い）。
iBGP は全て `update-source Loopback0` なので、**1.7 未完了だと全 iBGP が Idle のまま**。
VPNv4 の next-hop 解決に LDP ラベルも要るため、1.7 は二重の意味で前提。
着手前に R1 で `sh ip ospf nei` が 3 行出ることを確認する。

### password の適用範囲（根拠は問題文 3 章のみ）

> 3. BGPセッションの認証 — Global SP #1内のすべてのBGPセッションを、次のパスワードを使用して保護すること。`CCIE!nfr4`

- 「保護」= TCP MD5 = `neighbor <ip> password <pw>`。BGP でパスワードを使う保護は他にない。
- 範囲「**Global SP #1 内の**」→ R1/R2 は BGP 禁止なので、対象は **PE 4 台フルメッシュ = 6 セッション**
  （「ルートリフレクタは使用しないこと」がフルメッシュの根拠）。
- MD5 は両端に設定するので **各 PE 3 行 × 4 台 = 12 行**。「6 セッションだから 6 行」ではない。
- CE（R11/R21/R61/R62/R70）は AS 65001〜65007 で SP #1 の外 → **対象外**。
  CE 側の初期コンフィグに `password` が 1 行もないことが裏付け（付けると即セッションダウン）。

### `send-community extended` の扱い

| ソース | 出現数 |
|---|---|
| 解答 HTML 全体（Task 1.2〜1.14 / P1・P2・P3） | **0 件** |
| `EI_v2.yaml` | **9 件**（R4/R5/R6 × 3 ピア）。全件が `activate` 直後・`address-family vpnv4` 内 |

`address-family ipv4 vrf fabd2`（CE 向け eBGP）には 0 件。
→ 作者は一度も打っていないのに事前設定 3 台には入っている = **IOS が vpnv4 AF で自動付加**する動作と整合。

**打たなくても動くが、打っておくのが安全。**打ち忘れると RT が付かず
「BGP は Established なのに `show ip route vrf fabd2` に経路が入らない」= RT 不一致と同じ症状になり、
切り分けに時間を溶かす。1 行のコストはゼロ。
...
**⚠️ 閾値は `80` の可能性が高い（問題文と解答資料が矛盾）**

問題文は「**80,000 を超えたら警告** / 100,000 を超えたら shutdown / 5 分後に自動再確立」。
`maximum-prefix <上限> <警告閾値%> restart <分>` の第 2 引数は**パーセント**なので:
...
解答資料は 3 箇所すべて `90`（RS 比較 HTML 1346 / 3768 行、Pattern 4 チートシート 2124 行）だが、
**問題文どおりなら `80`。本番は問題文の数字を優先すること。**

**`maximum-prefix` 自体は Pattern 2 のみの追加要件**（original 1346 行 = P2 列）。

引数と問題文の対応（1 行で 3 要件を満たす）:

| 問題文の要件 | 引数 |
|---|---|
| 80,000 を超えたら警告メッセージ | `80` ← **パーセント指定。本数ではない** |
| 100,000 を超えたらセッションを shutdown | `100000` |
| 5 分後に自動再確立 | `restart 5`（省くと手動 `clear ip bgp` まで復旧しない） |

- **`warning-only` は付けない**。付けると警告のみでセッションを落とさず、要件を満たさない。
- **R21（100.3.21.2）向けだけ**。問題文が「R3 が **Data Center ルータ**から受信する」と限定しているため。
  HQ 側の R11（100.3.11.2）に付けるのは要件外。
