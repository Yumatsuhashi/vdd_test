...
このファイルは、本リポジトリでコードを記述する際の Codex (Codex.ai/code) へのガイドラインです。
...
├── .Codex/             # VDD Framework 設定（フック・スキル・仕様）
...
## Codex Hooks <!-- FRAMEWORK: DO NOT MODIFY -->

AGENTS.md のルールを技術的に強制するフック群。`.Codex/hooks/` に配置。設定は `.Codex/settings.json` の `hooks` セクションで定義。
...
| L2 | プロンプト内ルール | AGENTS.md に記載のみ。技術的強制なし |
...
- ツール非依存の正本:
  ...
  - `process/VDD.md`
- この `AGENTS.md`:
  - Codex で運用するための具体設定（フック、スキル、強制ルール）
  - リポジトリ内での実装パスや運用手順
...
- レビュー結果・PR コメント
- リリース仕様書（`.Codex/release-specs/{release-name}.md`）
- コミットメッセージの本文（件名は英語可）
...
- 成果物: リリース仕様書（`.Codex/release-specs/{release-name}.md`）
...
- リリース仕様書を `.Codex/release-specs/{release-name}.md` としてコミット
- TDD サイクルで実装
- リリース仕様書の範囲外の変更をしない
...
### プランファイル（`.Codex/plans/`）のコミット

プランファイルは過去の設計経緯を辿るために git 管理対象とする。コミット時に `.Codex/plans/` 内のファイルがあれば積極的に含めること。

### PR description の図解（必須）

リリース PR の description には必ず図解を含めること。AI はレビュアーの認知特性プロファイル（`.Codex/reviewer-profile.md`）を参照し、最適な表現方法を選択する。
...
- **メモリ変更のコミット（必須）**: サブエージェントがエージェントメモリ（`.Codex/agent-memory/`）を更新した場合、その変更を必ずコミットに含めること
...
## CCIE 学習資料と QA <!-- CUSTOMIZE -->

静的 Web アプリとは別に、CCIE ラボ学習用の資料と Q&A 蓄積の仕組みがある。
...
- **QA 運用ルールの正本は `.Codex/skills/ccie-qa/SKILL.md`**。パターンN・問題Xへの質問（設計理由・config の意図など）を受けると ccie-qa スキルが起動し、資料を突き合わせて回答したうえで `QA/patternN/YYYY-MM-DD_qX_<概要>.md` に蓄積する。
