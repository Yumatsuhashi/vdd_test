...
> **この yaml は CML 上で稼働中のラボ `EI_v2` そのもの**（ユーザーはこのラボで学習している）。
> yaml が**正本＝問題の初期状態の基準**で、実機側の差分は学習作業によるドリフト。
> 資料だけで答えが確定せず実機を確認したくなったら、**`cml-ops` スキルを読んでから** CML の
> MCP ツールを使う。**書き込み系は全て都度承認**（読み取りのみ承認不要）。
...
- **実機 CML の状態を勝手に変えて検証しない。** show 系での観測は自由だが、config 投入・
  ノード再起動・wipe はユーザーの承認を得てから（詳細は `cml-ops` スキル）。
...
（実機を使わなかった場合はセクションごと省略。使った場合は
対象ラボ/ノード・実行コマンド・show 出力・結論を書く。
状態を変更したなら「何を変えて戻したか／戻していないか」も必ず明記する）
